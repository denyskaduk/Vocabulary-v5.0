--QA with minor issues, anyway working
  ...
  union
    select distinct a.concept_code_1, 'same concept_code_1 precedence pair' from relationship_to_concept a 
  join relationship_to_concept b on a.CONCEPT_CODE_1 =b.CONCEPT_CODE_1 and a.CONCEPT_ID_2 != b.concept_id_2 and a.precedence =b.precedence
  union
...
union
--'zero equal ds_values'

select drug_concept_code, 'zero equal ds_values' from ds_stage where amount_value=0 or numerator_value = 0 or numerator_value = 0
 union
  select drug_concept_code, 'ds_stage unit absent in relationship_to_concept' from ds_stage where
  (
   numerator_unit not in (select concept_code_1 from relationship_to_concept) or amount_unit not in (select concept_code_1 from relationship_to_concept) or DENOMINATOR_UNIT not in (select concept_code_1 from relationship_to_concept)
)
) a join drug_concept_stage b on a.concept_code = b.concept_code where b.invalid_reason is null and b.domain_id = 'Drug'  
   group by error_type
;
select concept_code, 'standard but invalid concept' as  from drug_concept_stage where standard_concept='S' and invalid_reason is not null
;
